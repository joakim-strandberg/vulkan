...
package body Aida.UTF8_Code_Point with SPARK_Mode is
   ...
   procedure Check is
   begin
      for I in Mapping'First..(Mapping'Last - 1) loop
         if Mapping (I).Code >= Mapping (I + 1).Code then
            Aida.Text_IO.Put_Line (Image (Mapping (I).Code) & " > " & Image (Mapping (I + 1).Code));
         end if;
      end loop;
   end Check;
   ...
   procedure Find (Code  : T;
     ...
     Global => null,
     Pre => (for all I in Mapping'Range => (for all J in I..Mapping'Last => Mapping (I).Code <= Mapping (J).Code));
   ...
   is
      ...
      Current : Code_Point;
   begin
--        pragma Assert (for all I in Mapping'First..(Mapping'Last - 1) => Mapping (I).Code < Mapping (I + 1).Code);
--        pragma Assume (for all I in Mapping'First..(Mapping'Last - 1) => Mapping (I).Code < Mapping (I + 1).Code);
...
--           pragma Loop_Invariant (From <= To);
--           pragma Loop_Variant (Decreases => To - From);
...
--              From := This + 1;
--           end if;
--           pragma Loop_Invariant (From <= To and From >= Mapping'First and To <= Mapping'Last);
--           pragma Loop_Variant (Decreases => To - From);
--        end loop;
   end Find;
