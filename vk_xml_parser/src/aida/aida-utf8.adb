package body Aida.UTF8 with SPARK_Mode is
   ...
   begin
      Code := UTF8_Code_Point (Character'Pos (Source (Pointer)));
      ...
      case Code is
         ...
         when 16#C2#..16#DF# => -- 2 bytes
            ...
            Code := UTF8_Code_Point (Character'Pos (Source (Pointer + 1)));
            Value   := Accum or (Code and 16#3F#);
            Pointer := Pointer + 2;
         when 16#E0# => -- 3 bytes
            Code := UTF8_Code_Point (Character'Pos (Source (Pointer + 1)));
            Accum := (Code and 16#3F#) * 2**6;
            Code := UTF8_Code_Point (Character'Pos (Source (Pointer + 2)));
            Value   := Accum or (Code and 16#3F#);
            Pointer := Pointer + 3;
         ...
         when 16#F0# => -- 4 bytes
            ...
            Code := UTF8_Code_Point (Character'Pos (Source (Pointer + 2)));
            Accum := Accum or (Code and 16#3F#) * 2**6;
            Code := UTF8_Code_Point (Character'Pos (Source (Pointer + 3)));
            Value   := Accum or (Code and 16#3F#);
            Pointer := Pointer + 4;
